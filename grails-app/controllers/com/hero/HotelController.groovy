...
class HotelController {
    ...
    def index(Integer max) {
		...
		def hotelList = shiroUser.hotels

		def hotelIdList = []
		
		for(int i = 0 ;i < hotelList.size(); i++)
		{
			hotelIdList.add(hotelList.getAt(i).id)
		}
		
		def hotelListByUser = Hotel.findAllByIdInList(hotelIdList)
		
		respond hotelListByUser, model:[hotelInstanceCount: hotelListByUser.size()]
    }
